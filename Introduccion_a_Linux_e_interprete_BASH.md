...
_Contestar las preguntas con las respuestas concretas obtenidas de la "terminal" más alguna oracion en tus palabras._
...
cp /home/bioinformatica/Documentos/docs\_Bioinformatica/protein.fa.gz /home/bioinformatica/Documentos/TP1
...
mv protein.fa human\_proteome.faa
...
1. **¿Por qué contamos cuántos caracteres "\>" hay en el archivo para saber la cantidad de proteínas?**

1. **¿Cuántas ATP-binding proteins hay en el archivo? ¿Cómo realiza esto? ¿Que hace "grep"?**

Podemos usar **"grep"** nuevamente, ejecutamos:

grep "ATP-binding" human\_proteome.faa

Para contarlas:

grep -c "ATP-binding" human\_proteome.faa

1. **¿Como hariamos para que lo que se ve por pantalla sea escrito a un nuevo archivo?**
...
grep "ATP-binding" human\_proteome.faa \> atp\_binding.headers
...
1. **¿Cómo hacemos para que la salida de pantalla pase a ser la entrada a un nuevo comando?**
...
grep "ATP-binding" human\_proteome.faa | cut -f 2 -d "|" \> atp\_binding.gis

Para ver las 10 primeras líneas del archivo de salida ejecutamos:

head atp\_binding.gis
...
sh Anaconda3-2020.07-Linux-x86\_64.sh
...
conda create -n analisis\_de\_secuencias blast ncbi-vdb entrez-direct diamond sra-tools emboss exonerate python perl
...
grep "LL.ND" human\_proteome.faa

Nota: El carácter "." Significa wildcard: puede ser cualquier carácter

1. **¿Cuántas secuencias tienen ese patrón?**

1. **El comando "** _ **grep"** _ **contesta la pregunta de cuantas secuencias tienen ese patrón? ¿o no? ¿De qué depende? (Como interpreta el resultado)**

fasta\_formatter -i human\_proteome.faa -w 0 \> mod\_human\_proteome.faa

¿Qué cambios se generaron en el archive? ¿Puede ahora usar el comando grep para contestar la pregunta anterior?

1. **Ahora ejecutamos la búsqueda usando el comando** _ **"fuzzpro"** _ **de emboss**
...
fuzzpro -sequence human\_proteome.faa -pattern LLxND -outfile llxnd\_human\_proteome.fuzzpro

Ojo: en el comando fuzzpro, el caracter "x" es usado como wildcard.

1. **¿Son iguales los resultados obtenidos por ambos comandos (**_ **grep** _ **y** _ **fuzzpro** _**)? ¿Por qué? ¿Por qué cree que importante la búsqueda de patrones?**
...
tail llxnd\_human\_proteome.fuzzpro

1. **¿Cómo podemos editar los archivos creados?**

Para editar cualquiera de los archivos podemos usar el comando_ **"nano"** _(para salir Ctrl+x) o _"__ **vim"** _.

nano llxnd\_human\_proteome.fuzzpro

1. **¿Cómo podemos ordenar la lista de gi numbers obtenida anteriormente?**

Usando el comando_ **"sort"** _, probar que ocurre si usamos las opciones_ **"-n"** _ y/o_ **"-r"** _:

sort atp\_binding.gis
...
efetch -db nucleotide -id NC\_045512 -format fasta \> coronavirus.fasta
...
1. **¿Qué información puede extraer?**

Ahora vamos a descargar los CDS, es decir los transcriptos que dan lugar a las proteínas.

efetch -db nucleotide -id MT066156 -format fasta\_cds\_na \> coronavirus\_CDS.fasta

ejecutar efetch –help

1. **¿Según esta información que debería cambiar en el comando para obtener las proteínas?**

Ejecute el comando que cree que funcionará y guarde la secuencia en el archivo coronavirus\_aa.fasta.

efetch -db nucleotide -id MT066156 -format fasta\_cds\_aa\> coronavirus\_aa.fasta

1. **¿Cuál es la proteína más larga? ¿Y cuál es la más corta?**

**Nota** : puede ayudarse con un comando usado en ejercicios anteriores

**Ejecute el comando:**

fastacomposition -f coronavirus\_aa.fasta -s \> composition.txt
